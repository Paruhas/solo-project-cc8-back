...
exports.getBankAccount = async (req, res, next) => {
  ...
  try {
    ...
    if (!bankAccount)
      return res.status(400).json({ message: "can't get this bankAccount" });
...
exports.editStatusBankAccount = async (req, res, next) => {
  try {
    ...
    if (bankAccount.dataValues.isDeleted === "DELETED")
      return res
        .status(400)
        .json({
          message:
            "this id status is 'DELETED'; you cannot change this status anymore",
        });
