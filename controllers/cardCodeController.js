...
exports.getAllCardCodes = async (req, res, next) => {
  try {
    ...
    if (!cardCodeStatusType[status]) {
      return res.status(400).json({ message: "can't get this cardCode; this cardCode's status is undefined"});
    };
...
exports.getCardCodeByProductId = async (req, res, next) => {
  try {
    ...
    if ( Boolean(status) === false || !status) {
      ...
      if (cardCodesByProduct.length === 0) return res.status(400).json({ message: "can't get this cardCode; no cardCode founded"});
...
exports.editCardCodeStatus = async (req, res, next) => {
  try {
    ...
    if (findCodeNumbers.length !== ids.length) {
      return res.status(400).json({message: "enter id in 'ids' is out of range"});
    }
    if ( !(cardCodeStatusType[codeStatus]) || !(cardCodeStatusType[changeTo]) ) {
      return res.status(400).json({message: "no cardCodeStatusType for this number; Please enter correct number 'codeStatus' or 'changeTo'"});
    }
    if ( (cardCodeStatusType[codeStatus]) === (cardCodeStatusType[changeTo]) ) {
      return res.status(400).json({message: "codeStatus and changeTo must not equal"});
    }
    for (key of findCodeNumbers) {
      if (key.codeStatus !== cardCodeStatusType[codeStatus]) {
        return res.status(400).json({ message: "codeStatus at cardCode id " + key.id + " is not " + cardCodeStatusType[codeStatus] })
      }  
    };
